...
function AuthCodeGrant(config, req, res, next, check) {
  ...
  runner(fns, this, function (err) {
    if (err && res.oauthRedirect) {
      var errorParm = '?error=';
      if (this.client.redirectUri.indexOf('?') != -1) {
        errorParm = '&error=';
      }
      // Custom redirect error handler
      res.redirect(self.client.redirectUri + errorParm + err.error +
...
function redirect (done) {
  ...
  this.res.redirect(this.client.redirectUri + codeParm + this.authCode);
