...
function AuthCodeGrant(config, req, res, next, check) {
  ...
  runner(fns, this, function (err) {
    if (err && res.oauthRedirect) {
      var redirectUri = Array.isArray(self.client.redirectUri) ? self.client.redirectUri[0] : self.client.redirectUri;
      var errorParm = '?error=';
      if (redirectUri.indexOf('?') != -1) {
        errorParm = '&error=';
      }
      // Custom redirect error handler
      res.redirect(redirectUri + errorParm + err.error +
...
function redirect (done) {
  var redirectUri = Array.isArray(this.client.redirectUri) ? this.client.redirectUri[0] : this.client.redirectUri;
  var codeParm = '?code=';
  if (redirectUri.indexOf('?') != -1) {
    codeParm = '&code=';
  }
  this.res.redirect(redirectUri + codeParm + this.authCode);
